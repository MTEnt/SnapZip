...
*   **Task-aware knowledge cards**: Optional indexing mode derives compact local cards for definitions, usage examples, imports, tests, and config files so experimental ranking profiles can use structured repo knowledge instead of only raw chunk text.
...
snapzip index --db-dir . --langs all --crawl . --knowledge-cards
```
...
Indexed knowledge cards are visible in `snapzip stats` after indexing with `--knowledge-cards` or `SNAPZIP_KNOWLEDGE_CARDS=1`. Experimental card-aware runtime scoring is kept behind `SNAPZIP_RANK_PROFILE=language-symbol-cards` or `SNAPZIP_KNOWLEDGE_CARD_RANKING=1` until it clears the same public benchmark guardrails as the published `language-symbol` profile.
